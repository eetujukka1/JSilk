...
describe("DefaultPageLoader", () => {
  ...
  describe("constructor", () => {
    ...
    it("should initialize with custom proxies and onSuccess callback", () => {
      ...
      const proxies = [new Proxy("http://proxy1:8080"), new Proxy("http://proxy2:8080")];
  ...
  describe("loadPage", () => {
    ...
    it("should escalate to dynamic loader when scoreHtml indicates escalation is needed", async () => {
      ...
      const staticPage = await loader.staticLoader.loadPage("https://ip.oxylabs.io/location");
      
      // Manually create a page with HTML that will trigger escalation
      const highScoreHtml = "<div id='app'></div><div id='root'></div><script>__REACT__</script><script>React.render()</script>";
      ...
      const originalLoadPage = loader.staticLoader.loadPage.bind(loader.staticLoader);
    ...
    it("should not escalate when HTML score is below threshold", async () => {
      ...
      const lowScoreHtml = "<html><head><title>Test</title></head><body><p>This is a simple page with enough text content to avoid triggering escalation signals. It contains plain HTML without any JavaScript frameworks, SPA root containers, or other indicators that would require dynamic loading. The content is substantial enough to pass the text length check.</p></body></html>";
    ...
    it("should handle errors from static loader", async () => {
      ...
      await expect(loader.loadPage("https://invalid-url-that-does-not-exist-12345.com")).rejects.toThrow();
      expect(spyDynamicLoad).not.toHaveBeenCalled();
    ...
    it("should handle errors from dynamic loader when escalation is triggered", async () => {
      ...
      const originalStaticLoad = loader.staticLoader.loadPage.bind(loader.staticLoader);
      loader.staticLoader.loadPage = jest.fn().mockResolvedValue(staticPageResult);
      spyDynamicLoad.mockRejectedValue(new Error("Failed to load page dynamically"));

      await expect(loader.loadPage(mockPage)).rejects.toThrow("Failed to load page dynamically");
